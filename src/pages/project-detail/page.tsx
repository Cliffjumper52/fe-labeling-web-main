...
export default function ProjectDetail() {
  ...
  const resolveNames = (list: TeamMember[], ids: string[]) => {
    return ids.map((item) => list.find((entry) => entry.id === item)?.name || item);
  };
  ...
  const applyMemberAssignments = (
    ...
    memberId: string,
    setAssignments: (updater: (prev: Record<string, string[]>) => Record<string, string[]>) => void,
  ) => {
    setAssignments((prev) => {
      const next = { ...prev };
  ...
  const pushTaskToAnnotatorQueue = async () => {
    const assignedAnnotatorNames = resolveNames(annotators, derivedAnnotatorIds);
    ...
    const basePayload = {
      ...
      checklist: ["All required labels are added", "Quality self-check completed"],
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1.2fr_1fr]">
          <div className="space-y-4">
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              ...
              {uploadedFiles.length === 0 ? (
                <p className="mt-2 text-xs text-gray-400">No files uploaded yet.</p>
              ) : (
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              <p className="text-sm font-semibold text-gray-700">Assign annotators</p>
              {uploadedFiles.length === 0 ? (
                <p className="mt-2 text-xs text-gray-400">Upload files first.</p>
              ) : (
                  ...
                  {selectedAnnotatorId ? (
                    <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                      <span className="text-xs font-semibold text-gray-700">
                        Selected: {resolveNames(annotators, [selectedAnnotatorId]).join(", ")}
                      </span>
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    <p className="text-xs text-gray-400">Select an annotator to assign.</p>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              <p className="text-sm font-semibold text-gray-700">Assign reviewers</p>
              {uploadedFiles.length === 0 ? (
                <p className="mt-2 text-xs text-gray-400">Upload files first.</p>
              ) : (
                <div className="mt-3 space-y-3">
                  ...
                  {selectedReviewerId ? (
                    <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                      <span className="text-xs font-semibold text-gray-700">
                        Selected: {resolveNames(reviewers, [selectedReviewerId]).join(", ")}
                      </span>
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    <p className="text-xs text-gray-400">Select a reviewer to assign.</p>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-gray-700">Select preset</p>
                {selectedPreset && (
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 space-y-2 text-sm text-gray-600">
                <p>
                  Annotators: {resolveNames(annotators, derivedAnnotatorIds).join(", ") || "Unassigned"}
                </p>
                <p>
                  Reviewers: {resolveNames(reviewers, derivedReviewerIds).join(", ") || "Unassigned"}
                </p>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/30 px-4">
          <div className="w-full max-w-lg rounded-lg border border-gray-300 bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-4 py-3">
              <h3 className="text-sm font-semibold text-gray-800">Assign files</h3>
              <button
                type="button"
            ...
            <div className="p-4">
              <p className="text-xs text-gray-500">
                Annotator: {resolveNames(annotators, [selectedAnnotatorId]).join(", ")}
              </p>
              <div className="mt-3 space-y-2 rounded-md border border-gray-200 p-3">
                ...
                {uploadedFiles.length > 0 &&
                  uploadedFiles.filter(
                    (file) => (annotatorFileAssignments[file] ?? []).length === 0,
                  ).length === 0 && (
      ...
      {isReviewerAssignOpen && selectedReviewerId && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/30 px-4">
          <div className="w-full max-w-lg rounded-lg border border-gray-300 bg-white shadow-xl">
            ...
            <div className="p-4">
              <p className="text-xs text-gray-500">
                Reviewer: {resolveNames(reviewers, [selectedReviewerId]).join(", ")}
              </p>
              <div className="mt-3 space-y-2 rounded-md border border-gray-200 p-3">
                {uploadedFiles.length === 0 ? (
                  <p className="text-xs text-gray-400">No files uploaded yet.</p>
