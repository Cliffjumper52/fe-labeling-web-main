    ...
    return {
      ...
      severity:
        ...
        matched.severity === "High"
          ? (matched.severity as Severity)
          : undefined,
      qaDecision:
        matched.qaDecision === "Approved" || matched.qaDecision === "Rejected"
          ? (matched.qaDecision as "Approved" | "Rejected")
          : undefined,
      qaReviewedAt:
        typeof matched.qaReviewedAt === "string" ? matched.qaReviewedAt : "",
      annotatorScoreDelta:
        typeof matched.annotatorScoreDelta === "number"
          ? matched.annotatorScoreDelta
          : undefined,
    };
  } catch {
    return null;
...
export default function ReviewerWorkspacePage() {
  ...
  >({});
  const [reviewOutcome, setReviewOutcome] = useState<"Approved" | "Returned">(
    task.status === "Completed" ? "Approved" : "Returned",
  );
  ...
  const feedbackForSubmit = (reviewFeedbackByKey[roundKey] ?? "").trim();

  const allCurrentErrorTypes = useMemo(() => {
    const fromRecorded = recordedReviewErrors.map(
      (error) => error.reviewErrorTypeName,
    );
    const fromTask = task.errorTypes ?? [];
    return Array.from(new Set([...fromTask, ...fromRecorded]));
  }, [recordedReviewErrors, task.errorTypes]);

  const highestSeverity = useMemo<Severity>(() => {
    const recordedSeverities = recordedReviewErrors.map((error) => error.severity);
    const candidates = task.severity
      ? [task.severity, ...recordedSeverities]
      : recordedSeverities;
    ...
    return candidates.reduce((highest, current) =>
      severityWeight[current] > severityWeight[highest] ? current : highest,
    );
  }, [recordedReviewErrors, task.severity]);
  ...
  useEffect(() => {
    ...
    setReviewOutcome(task.status === "Completed" ? "Approved" : "Returned");
  ...
  const handleSendFeedbackToAnnotator = () => {
    if (typeof window === "undefined") {
      return;
    }

    if (reviewOutcome === "Returned" && !feedbackForSubmit) {
      toast.error("Please enter feedback before sending back to annotator.");
      return;
    }

    const raw = localStorage.getItem(ANNOTATOR_TASKS_STORAGE_KEY);
    if (!raw) {
      toast.error("Task storage not found.");
      return;
    }
    ...
    try {
      ...
      const next = tasks.map((item) => {
        ...
        if (reviewOutcome === "Approved") {
          return {
            ...
            reviewerNote: feedbackForSubmit || "Approved by reviewer.",
            errorTypes: [],
            severity: undefined,
        ...
        return {
          ...
          qaDecision: "Rejected",
          qaReviewedAt: reviewedAt,
          reviewerNote: feedbackForSubmit,
          errorTypes: allCurrentErrorTypes,
          severity: highestSeverity,
          annotatorScoreDelta: scoreFromSeverity(highestSeverity),
        };
      });

      if (!found) {
        toast.error("Task not found in assigned queue.");
        return;
      }
      ...
      toast.success(
        reviewOutcome === "Approved"
          ? "Review approved and sent to annotator history."
          : "Feedback sent back to annotator.",
      );
    } catch {
      toast.error("Failed to save reviewer feedback.");
    }
  };
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-2 grid grid-cols-1 gap-2">
              ...
              <select
                value={reviewOutcome}
                onChange={(event) =>
                  setReviewOutcome(event.target.value as "Approved" | "Returned")
                }
                title="Review outcome"
                className="rounded-md border border-gray-300 px-3 py-2 text-sm"
              >
                <option value="Returned">Returned to annotator</option>
            ...
            <div className="mt-3 grid grid-cols-1 gap-2">
              ...
              <button
                type="button"
                onClick={handleSendFeedbackToAnnotator}
                className="rounded-md bg-blue-600 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700"
              >
                Send feedback to annotator
              </button>
            </div>
